...
{
    char temp;
    // m_read_idx指向缓冲区m_read_buf的数据末尾的下一个字节,m_checked_idx指向从状态机当前正在分析的字节
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    *m_version++ = '\0';
    ...
    // 对请求资源前7个字符进行判断,这里主要是有些报文的请求资源中会带有http://，这里需要对这种情况进行单独处理
...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    // 避免将用户名和密码直接暴露在URL中，我们在项目中改用了POST请求，将用户名和密码添加在报文中作为消息体进行了封装
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        if (temp < 0) // 发送失败
        {
            if (errno == EAGAIN) // 若是因为缓冲区已满，则注册写事件
            {
